...
[[ -n ${sonarr_api_key} && -n ${prowlarr_api_key} && -n ${radarr_api_key} ]] || die 'An Arr API key is missing.'
...
echo STEP_RADARR_ROOT_FOLDER
arr_get radarr 7878 v3 "${radarr_api_key}" rootfolder "${work_dir}/radarr-rootfolders.json"
if ! python3 - "${work_dir}/radarr-rootfolders.json" <<'PY'
import json, sys
with open(sys.argv[1], encoding="utf-8") as handle:
    raise SystemExit(0 if any(item.get("path") == "/data/media/Movies" for item in json.load(handle)) else 1)
PY
then
  printf '%s' '{"path":"/data/media/Movies"}' >"${work_dir}/radarr-rootfolder-payload.json"
  arr_write radarr 7878 v3 "${radarr_api_key}" POST rootfolder \
    "${work_dir}/radarr-rootfolder-payload.json" "${work_dir}/radarr-rootfolder-response.json"
fi

echo STEP_RADARR_QBITTORRENT
arr_get radarr 7878 v3 "${radarr_api_key}" downloadclient/schema "${work_dir}/radarr-downloadclient-schema.json"
arr_get radarr 7878 v3 "${radarr_api_key}" downloadclient "${work_dir}/radarr-downloadclients.json"
radarr_download_client_id="$(resource_id "${work_dir}/radarr-downloadclients.json" qBittorrent)"
MEDIA_USER="${media_user}" MEDIA_PASSWORD="${media_password}" RESOURCE_ID="${radarr_download_client_id}" \
  python3 - "${work_dir}/radarr-downloadclient-schema.json" "${work_dir}/radarr-downloadclient-payload.json" <<'PY'
import json, os, sys
with open(sys.argv[1], encoding="utf-8") as handle:
    item = next(value for value in json.load(handle) if value.get("implementation") == "QBittorrent")
item.update({"enable": True, "name": "qBittorrent", "priority": 1,
...
values = {
    "host": "qbittorrent", "port": 8080, "useSsl": False,
    "username": os.environ["MEDIA_USER"], "password": os.environ["MEDIA_PASSWORD"],
    "movieCategory": "movies-radarr", "initialState": 0,
    "sequentialOrder": False, "firstAndLast": False, "contentLayout": 0,
}
for field in item["fields"]:
    if field.get("name") in values:
...
with open(sys.argv[2], "w", encoding="utf-8") as handle:
    json.dump(item, handle)
PY
if [[ -n ${radarr_download_client_id} ]]; then
  arr_write radarr 7878 v3 "${radarr_api_key}" PUT "downloadclient/${radarr_download_client_id}" \
    "${work_dir}/radarr-downloadclient-payload.json" "${work_dir}/radarr-downloadclient-response.json"
else
  arr_write radarr 7878 v3 "${radarr_api_key}" POST downloadclient \
    "${work_dir}/radarr-downloadclient-payload.json" "${work_dir}/radarr-downloadclient-response.json"
fi
...
echo STEP_PROWLARR_RADARR
arr_get prowlarr 9696 v1 "${prowlarr_api_key}" applications "${work_dir}/applications.json"
radarr_application_id="$(resource_id "${work_dir}/applications.json" Radarr)"
RADARR_API_KEY="${radarr_api_key}" RESOURCE_ID="${radarr_application_id}" \
  python3 - "${work_dir}/application-schema.json" "${work_dir}/radarr-application-payload.json" <<'PY'
import json, os, sys
with open(sys.argv[1], encoding="utf-8") as handle:
    item = next(value for value in json.load(handle) if value.get("implementation") == "Radarr")
item.update({"enable": True, "name": "Radarr", "syncLevel": "fullSync"})
values = {
...
with open(sys.argv[2], "w", encoding="utf-8") as handle:
    json.dump(item, handle)
PY
if [[ -n ${radarr_application_id} ]]; then
  arr_write prowlarr 9696 v1 "${prowlarr_api_key}" PUT "applications/${radarr_application_id}" \
    "${work_dir}/radarr-application-payload.json" "${work_dir}/radarr-application-response.json"
else
  arr_write prowlarr 9696 v1 "${prowlarr_api_key}" POST applications \
    "${work_dir}/radarr-application-payload.json" "${work_dir}/radarr-application-response.json"
fi
